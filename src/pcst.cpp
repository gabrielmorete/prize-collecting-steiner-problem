...
int solve(Graph G, string nme, GRBEnv &env){
	...
	auto duration = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() * 1e-9;
...
int main(){
	...
	printf("P : %.3e %.3e %d\n", tme_avg / ((double) n_inst), tme_mx, n_inst);
