...
typedef vector< vector<pair<int, int> > > matrixi;

// returns the # of visited nodes
int dfs(int v, int *vis, double *y, vector<int> adj[]){
	vis[v] = 1;

	int tree = 1;
	for (int u : adj[v])
		if (y[u] > 0.5 and !vis[u])
			tree += dfs(u, vis, y, adj);
...
class cut_tree: public GRBCallback{
	public:
		...
		matrixi adj;
		int n;
		cut_tree(GRBVar* _y, GRBVar** _x, int _n, matrixi _adj) {
			...
			try {
				if (where == GRB_CB_MIPSOL) {
					...
					for (int v = 0; v <= n; v++){
						if (y[v] > 0.5){
							for (int u = 1; u <= n; u++){
								if (getSolution(varx[v][u]) > 0.5 and y[u] > 0.5)
									x[v].push_back(u);
							}
						}
					}
					...
					if (dfs(0, vis, y, x) < nvtx){
						GRBLinExpr cut = 0;
					
						for (int v = 0; v <= n; v++){
							if (vis[v]){ // terminal inside the cut
								for (int u = 1; u <= n; u++){
									if (!vis[u])
										cut += varx[v][u];
								}
...
int main(){
	...
	string file_name = "../instances/PCSPG-JMP/K200.stp";
	...
	try {
		...
		// Delete single non-costumer nodes
		for (int v = 0; v <= n; v++)
			if (!term[v] and G.adj[v].empty())
		...
		cut_tree cb = cut_tree(y, x, n, G.adj);
