...
typedef vector< vector< type_val> >  vvt;
typedef vector< type_val> vt;


void dijkstra(int src, int n, vvit adj, vt &dist){
	for (int v = 1; v <= n; v++)
		dist[v] = -1;
...
bool least_cost(int &n, int &m, vvit &adj, vvt &dist){
	bool ok = 0;
...
void preprocessing(Graph &G){
	...
	vvt dist(n + 1, vector<type_val>(n + 1));

	least_cost(n, m, adj, dist);
